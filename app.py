...
# Store scanned QR code result globally
if "scanned_text" not in st.session_state:
    st.session_state.scanned_text = None

# Class for real-time QR code scanning using OpenCV
class QRCodeScanner(VideoTransformerBase):
    def transform(self, frame):
        ...
        if data:
            st.session_state.scanned_text = data  # Store result globally
...
st.markdown("### Generate, decode, and scan QR codes in real-time.")
...
if st.session_state.scanned_text:
    st.success(f"Scanned QR Code: {st.session_state.scanned_text}")
